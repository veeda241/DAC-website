...
const Dashboard: React.FC<DashboardProps> = ({
  ...
  return (
    <div className="min-h-screen bg-transparent text-slate-200 flex relative overflow-hidden selection:bg-purple-500/30">
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 bg-[#061222]/90 backdrop-blur-xl border-t border-[#EE002D]/30 md:hidden pb-safe">
        <div className="flex justify-around items-center p-2">
          {[
            ...
            >
              <tab.icon className={`w-6 h-6 mb-1 ${activeTab === tab.id ? 'drop-shadow-[0_0_8px_rgba(227,6,19,0.5)]' : ''}`} />
              <span className="text-[10px] font-medium">{tab.label}</span>
            </button>
          ))}
          <button
            onClick={() => setActiveTab('settings')}
            className={`flex flex-col items-center p-2 rounded-lg transition-all ${activeTab === 'settings' ? 'text-purple-500' : 'text-slate-400 hover:text-slate-200'
              }`}
          >
            <Settings className={`w-6 h-6 mb-1 ${activeTab === 'settings' ? 'drop-shadow-[0_0_8px_rgba(227,6,19,0.5)]' : ''}`} />
      ...
      {/* Sidebar (Desktop) */}
      <aside className={`w-64 bg-[#061222]/80 backdrop-blur-xl border-r border-[#EE002D]/30 hidden md:flex flex-col fixed h-full z-30 transition-transform duration-300 md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-6 border-b border-slate-800/50 flex justify-between items-center">
          <div className="flex items-center gap-3">
            <img src={LOGO_URL} alt="Logo" className="w-8 h-8 object-contain" />
            <h2 className="text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-purple-800">
              Portal
            </h2>
          </div>
          ...
          {[
            ...
            <button
              ...
              className={`w-full text-left px-4 py-3 rounded-xl flex items-center gap-3 transition-all duration-200 ${activeTab === tab.id
                ? 'bg-purple-500/10 text-purple-500 border border-purple-500/20 shadow-[0_0_15px_rgba(227,6,19,0.1)]'
        ...
        <div className="p-4 border-t border-slate-800/50 bg-slate-900/30">
          ...
          <button onClick={onLogout} className="w-full flex items-center justify-center gap-2 text-purple-400 hover:text-white hover:bg-purple-500/20 p-2 rounded-lg transition-colors text-sm font-medium">
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-4 py-2 bg-slate-900 border border-slate-800 rounded-lg text-sm text-white focus:outline-none focus:border-purple-500 sm:w-64 transition-all"
                  />
                </div>
              </div>
            )}
          ...
          {activeTab === 'overview' && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fade-in-up delay-100">
              ...
              {[
                ...
                { label: 'Active Tasks', val: tasks.filter(t => t.status !== TaskStatus.COMPLETED).length, color: 'text-purple-500', bg: 'bg-gradient-to-br from-slate-800 to-slate-900' },
                { label: 'Published Reports', val: reports.length, color: 'text-emerald-400', bg: 'bg-gradient-to-br from-slate-800 to-slate-900' }
              ].map((stat, i) => (
                <div key={i} className={`${stat.bg} p-6 rounded-2xl border border-white/5 shadow-xl relative overflow-hidden group hover:border-purple-500/30 transition-all`}>
                      ...
                      .map((event, index) => (
                        <div key={event.id} className="flex gap-4 p-4 rounded-xl bg-white/5 border border-white/5 hover:border-purple-500/30 transition-all group">
                          ...
                          <div className="flex-1 min-w-0">
                            <h4 className="font-semibold text-white truncate group-hover:text-purple-400 transition-colors">{event.title}</h4>
              ...
              <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-xl overflow-hidden flex flex-col">
                <h3 className="text-lg font-semibold mb-4 flex items-center gap-2"><Bell className="w-5 h-5 text-slate-500" /> Recent Activity</h3>
              ...
              {/* AI Insights */}
              <div className="lg:col-span-3 bg-gradient-to-r from-purple-900/40 to-slate-900 p-6 rounded-2xl border border-purple-500/30 shadow-xl relative overflow-hidden">
                ...
                <div className="flex items-center justify-between mb-4 relative z-10">
                  ...
                  <button
                    ...
                    className="text-xs bg-purple-600 hover:bg-purple-500 text-white px-4 py-2 rounded-full flex items-center gap-2 disabled:opacity-50 transition-all shadow-lg shadow-purple-900/20"
                  >
                    ...
                    <div className="whitespace-pre-line animate-fade-in-up pr-24 relative group">
                      ...
                      <button
                        onClick={() => {
                          ...
                          downloadAsPDF("RED BULL AI INSIGHTS", aiAnalysis, `DAC_Analysis_${new Date().toISOString().split('T')[0]}.pdf`);
                        }}
                        className="absolute top-0 right-0 bg-purple-500/20 hover:bg-purple-500 text-purple-100 hover:text-white p-2 rounded-lg transition-all md:opacity-0 md:group-hover:opacity-100 flex items-center gap-2 text-xs"
                      >
            ...
            <div className="animate-fade-in-up">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {filteredEvents.map(event => (
                  <div key={event.id} className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden flex flex-col group hover:border-purple-500/30 transition-all duration-300">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {users.map(member => (
                  <div
                    ...
                    className="bg-slate-900 border border-slate-800 rounded-2xl p-6 hover:border-purple-500/30 transition-all group cursor-pointer"
                  >
                    <div className="flex flex-col items-center text-center">
                      <div className="relative">
                        <img
                          src={member.avatar}
                          alt={member.name}
                          className="w-24 h-24 rounded-full mb-4 border-4 border-slate-800 group-hover:border-purple-500/50 transition-all"
                        />
                  ...
                  <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-2xl p-8 shadow-2xl">
                    ...
                    <form onSubmit={(e) => {
                      ...
                      <div className="flex items-center gap-6">
                        ...
                        <div className="flex-1">
                          ...
                          <input
                            ...
                            className="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-white focus:outline-none focus:border-purple-500"
                      ...
                      <div className="flex gap-3 pt-4">
                        <button
                          type="submit"
                          className="flex-1 bg-purple-600 hover:bg-purple-700 text-white px-6 py-3 rounded-lg font-semibold transition-all hover:scale-105"
                        >
                          Save Changes
                        </button>
                        <button
              ...
              <div className="grid md:grid-cols-3 gap-6">
                ...
                <div className="md:col-span-2 space-y-4">
                  <h3 className="text-lg font-bold text-white mb-2 flex items-center gap-2">
                    <FileText className="w-5 h-5 text-purple-500" /> Published Reports
                  </h3>
                  ...
                  <div className="space-y-3">
                    {reports.map(report => (
                      <div key={report.id} className="group bg-slate-900 border border-slate-800 hover:border-purple-500/30 rounded-xl p-3 flex gap-4 transition-all hover:bg-slate-800/50">
                        {/* Thumbnail */}
                        ...
                        <div className="flex-1 min-w-0 flex flex-col justify-center">
                          <div className="flex justify-between items-start gap-2">
                            <div>
                              <h4 className="font-semibold text-white group-hover:text-purple-500 transition-colors truncate pr-4">
                                {report.title}
                              </h4>
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              {canManageContent && (
                                <>
                                  ...
                                  }} className="p-2 text-slate-400 hover:text-slate-400 hover:bg-slate-500/10 rounded-lg transition-colors" title="Edit">
                                  ...
                                  }} className="p-2 text-slate-400 hover:text-purple-400 hover:bg-purple-500/10 rounded-lg transition-colors" title="Delete">
                                    <Trash2 className="w-4 h-4" />
                                  </button>
                                </>
                              )}
                  ...
                  <div className="md:col-span-1">
                    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-lg sticky top-6">
                      <h3 className="text-lg font-bold text-white mb-6 flex items-center gap-2 border-b border-slate-800 pb-4">
                        {editingReportId ? <Edit className="w-5 h-5 text-purple-500" /> : <Upload className="w-5 h-5 text-slate-500" />}
                      ...
                      <form onSubmit={async (e) => {
                        ...
                        <div>
                          ...
                          <input required type="text" value={reportTitle} onChange={e => setReportTitle(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-white focus:outline-none focus:border-purple-500 transition-colors" placeholder="Report Title" />
                        </div>

                        <div>
                          <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Event (Optional)</label>
                          <select value={reportEventId} onChange={e => setReportEventId(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-white focus:outline-none focus:border-purple-500 transition-colors">
                        ...
                        <div>
                          ...
                          <textarea
                            ...
                            className="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-white focus:outline-none focus:border-purple-500 resize-none transition-colors"
                            rows={3}
                        ...
                        <div>
                          ...
                          <div className="relative">
                            <input type="file" onChange={handleFileChange} accept=".pdf,.doc,.docx" className="w-full text-xs text-slate-400 file:mr-3 file:py-2 file:px-3 file:rounded-md file:border-0 file:text-xs file:font-semibold file:bg-slate-800 file:text-purple-500 hover:file:bg-slate-700 cursor-pointer" required={!editingReportId} />
                        ...
                        <div className="flex gap-2 pt-2">
                          ...
                          <button type="submit" className={`flex-1 font-bold py-2.5 rounded-lg transition-all hover:scale-[1.02] text-sm ${editingReportId ? 'bg-purple-600 hover:bg-purple-700 text-white' : 'bg-purple-600 hover:bg-purple-700 text-white'}`}>
                ...
                {canManageContent && (
                  <div className="md:col-span-1 bg-slate-900 border border-slate-800 rounded-2xl p-6 h-fit shadow-lg">
                    <h3 className="text-lg font-bold text-white mb-6 flex items-center gap-2">
                      <ImageIcon className="w-5 h-5 text-purple-500" /> Add to Gallery
                    </h3>
                    <form onSubmit={handleUploadPhoto} className="space-y-4">
                      <div>
                        <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Caption</label>
                        <input required type="text" value={photoCaption} onChange={e => setPhotoCaption(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-white focus:outline-none focus:border-purple-500" placeholder="e.g. Winners of Hackathon" />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Related Event</label>
                        <select required value={photoEventId} onChange={e => setPhotoEventId(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-white focus:outline-none focus:border-purple-500">
                      ...
                      <div>
                        ...
                        <input type="file" onChange={handlePhotoFileChange} accept="image/*" multiple className="w-full text-xs text-slate-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-purple-500/10 file:text-purple-500 hover:file:bg-purple-500/20 cursor-pointer" required />
                      </div>
                      ...
                      <button type="submit" className="w-full bg-purple-600 hover:bg-purple-700 text-white font-bold py-3 rounded-lg transition-all hover:scale-[1.02] mt-2">
                        Upload Photo
                      </button>
                    </form>
                  </div>
                    ...
                    {photos.map(photo => (
                      <div key={photo.id} className="relative group rounded-xl overflow-hidden aspect-square border border-slate-800">
                        ...
                        <div className="absolute inset-0 bg-gradient-to-t from-slate-900 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity p-4 flex flex-col justify-end">
                          ...
                          <span className="text-[10px] text-purple-300">{getEventName(photo.eventId)}</span>
                        </div>
                        {canManageContent && (
                          <button
                            onClick={() => {
                            ...
                            className="absolute top-2 right-2 p-1.5 bg-purple-500/80 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity hover:bg-purple-600"
                          >
                            <Trash2 className="w-4 h-4" />
                          </button>
                        )}
            ...
            <div className="animate-fade-in-up max-w-2xl mx-auto">
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 shadow-xl">
                <h2 className="text-xl font-bold mb-6 flex items-center gap-2">
                  <Settings className="w-5 h-5 text-purple-500" /> Account Settings
                </h2>
                ...
                <form onSubmit={handleSaveSettings} className="space-y-6">
                  ...
                  <div className="flex items-center gap-6 pb-6 border-b border-slate-800">
                    ...
                    <div className="flex-1">
                      ...
                      <input
                        ...
                        className="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-white focus:outline-none focus:border-purple-500 text-sm"
                  ...
                  <div className="pt-4 flex justify-end">
                    <button type="submit" className="bg-purple-600 hover:bg-purple-700 text-white px-6 py-3 rounded-xl font-semibold flex items-center gap-2 shadow-lg shadow-purple-900/20 transition-all hover:scale-105">
      ...
      {/* Create Event Modal - Enhanced UI */}
      {
        isEventModalOpen && (
          ...
          >
            <div
              className="bg-[#0A0A0C] rounded-[2rem] w-full max-w-2xl border border-white/10 shadow-2xl shadow-purple-500/10 relative overflow-hidden my-auto"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Modal Header with Gradient */}
              <div className="relative h-24 bg-gradient-to-r from-purple-600 via-purple-800 to-slate-600 overflow-hidden">
              ...
              <form onSubmit={handleCreateEvent} className="p-6 space-y-5">
                ...
                <div className="group">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-900/50 border border-white/10 rounded-xl p-4 text-white text-lg focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 transition-all placeholder:text-slate-600"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-slate-900/50 border border-white/10 rounded-xl p-3 text-white focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 transition-all placeholder:text-slate-600"
                      placeholder="6:00 PM"
                    />
                  </div>
                  <div>
                ...
                <div>
                  <label className="flex items-center gap-2 text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">
                    <span className="w-5 h-5 rounded bg-slate-500/10 flex items-center justify-center">
                ...
                <div>
                  ...
                  <div className="flex gap-4">
                    <div className="flex-1">
                      <label className="flex items-center justify-center gap-2 p-4 bg-slate-900/50 border-2 border-dashed border-white/10 hover:border-purple-500/50 rounded-xl cursor-pointer transition-all group">
                        <Upload className="w-5 h-5 text-slate-500 group-hover:text-purple-500 transition-colors" />
                        <span className="text-sm text-slate-500 group-hover:text-purple-500 transition-colors">
                ...
                <div>
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 px-3 py-1.5 bg-gradient-to-r from-purple-500/10 to-slate-500/10 hover:from-purple-500/20 hover:to-slate-500/20 border border-purple-500/20 rounded-full text-xs text-purple-500 font-medium transition-all disabled:opacity-50"
                    >
                  ...
                  <textarea
                    ...
                    className="w-full bg-slate-900/50 border border-white/10 rounded-xl p-4 text-white h-28 focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 transition-all placeholder:text-slate-600 resize-none"
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button
                    type="submit"
                    className="flex-1 px-6 py-3 bg-gradient-to-r from-purple-600 to-purple-800 hover:from-purple-500 hover:to-purple-700 rounded-xl text-white font-bold transition-all hover:shadow-lg hover:shadow-purple-500/25 flex items-center justify-center gap-2"
                  >
            ...
            <div className="bg-slate-900 p-6 rounded-2xl w-full max-w-md border border-slate-700 shadow-2xl relative">
              ...
              <form onSubmit={handleCreateTask} className="space-y-4">
                <div>
                  ...
                  <input required type="text" value={newTaskTitle} onChange={(e) => setNewTaskTitle(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-white focus:outline-none focus:border-purple-500" placeholder="e.g. Design Poster" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Related Event</label>
                  <select required value={newTaskEventId} onChange={(e) => setNewTaskEventId(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-white focus:outline-none focus:border-purple-500">
                ...
                <div>
                  ...
                  <select value={newTaskAssigneeId} onChange={(e) => setNewTaskAssigneeId(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-white focus:outline-none focus:border-purple-500">
                ...
                <button type="submit" className="w-full bg-purple-600 hover:bg-purple-700 text-white font-bold py-3 rounded-lg transition-all hover:scale-[1.02]">Create Task</button>
              </form>
            </div>
          </div>
        )
...
const TaskCard: React.FC<{
  ...
  return (
    <div className={`p-4 rounded-xl border transition-all duration-200 group relative ${isAssignedToMe ? 'bg-purple-900/10 border-purple-500/30 hover:border-purple-500' : 'bg-slate-800 border-slate-700 hover:border-slate-600'}`}>
      <div className="flex justify-between items-start mb-2">
        ...
        {isAssignedToMe && <span className="text-[10px] text-purple-400 font-bold bg-purple-900/30 px-2 py-1 rounded">YOU</span>}
        {canManageContent && ( // Conditionally render delete button
          <button
            ...
            className="p-1 text-slate-500 hover:text-purple-400 hover:bg-purple-500/10 rounded-lg transition-colors"
            title="Delete Task"
          >
            <Trash2 className="w-4 h-4" />
          </button>
      ...
      <div className="flex items-center gap-2 mb-4">
        <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${task.assigneeId ? 'bg-purple-600 text-white' : 'bg-slate-700 text-slate-400'}`}>
      ...
      <div className="flex gap-2 opacity-100 transition-opacity">
        ...
        {task.status === TaskStatus.PENDING && (
          <button onClick={() => onStatusChange(task.id, TaskStatus.IN_PROGRESS)} className="text-xs flex-1 py-1.5 bg-purple-600 hover:bg-purple-500 text-white rounded font-medium shadow-lg shadow-purple-900/20 transition-all hover:scale-105">Start Task</button>
        )}
